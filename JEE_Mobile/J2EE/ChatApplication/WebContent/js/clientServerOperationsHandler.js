...
function ClientServerOperationsHandler(ch, m){
	...
	ClientServerOperationsHandler.prototype.createConnection = function(){
		...
		try{
			var params = escape("channel") + "=" + escape(this.channel) +"&"+ escape("userName")+ "=" + escape($("#userName").val()) +"&"+ escape("password")+ "=" + escape($("#password").val());
			var url = "/SpringHibernateChatApplication/login?" + params;
	...
	ClientServerOperationsHandler.prototype.sendMessage = function (message){
		...
		try{
			...
			$.ajax({
				  ...
				  url  : '/SpringHibernateChatApplication/publish',
